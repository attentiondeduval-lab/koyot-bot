...
PHOTOS = {
    ...
    "cheese_roll":  "https://i.ibb.co/xqrfCgyT/IMG-5928.jpg",
    "ham_roll":     "https://i.ibb.co/1tpQvF50/IMG-5927.jpg",
    "toast_ham":    "https://i.ibb.co/R4H99131/IMG-5926.jpg",
    "toast_chick":  "https://i.ibb.co/twGZv8kb/IMG-5925.jpg",
}
...
async def big_lavash(callback: types.CallbackQuery):
    ...
    builder = InlineKeyboardBuilder()
    for item in items:
        builder.button(text=f"{item['name']} — {item['price']} ₴", callback_data=f"dish_{item['id']}_big_lavash")
...
async def big_bulka(callback: types.CallbackQuery):
    ...
    builder = InlineKeyboardBuilder()
    for item in items:
        builder.button(text=f"{item['name']} — {item['price']} ₴", callback_data=f"dish_{item['id']}_big_bulka")
    builder.button(text="🔙 Назад", callback_data="menu_big")
    builder.adjust(1)
    await callback.message.edit_text(
        "🔴 БІГ МЕНЮ · 🍞 *В булці*\n\nОбери страву:",
...
async def mid_lavash(callback: types.CallbackQuery):
    ...
    builder = InlineKeyboardBuilder()
    for item in items:
        builder.button(text=f"{item['name']} — {item['price']} ₴", callback_data=f"dish_{item['id']}_mid_lavash")
...
async def mid_bulka(callback: types.CallbackQuery):
    ...
    builder = InlineKeyboardBuilder()
    for item in items:
        builder.button(text=f"{item['name']} — {item['price']} ₴", callback_data=f"dish_{item['id']}_mid_bulka")
    builder.button(text="🔙 Назад", callback_data="menu_mid")
    builder.adjust(1)
    await callback.message.edit_text(
...
async def show_dish(callback: types.CallbackQuery):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Хочу замовити!", callback_data=f"order_{item_id}")
...
async def r_hungry(callback: types.CallbackQuery):
    builder = InlineKeyboardBuilder()
    builder.button(text="🌯 Люблю лаваш",   callback_data="dish_coyote_big_lavash")
    builder.button(text="🍞 Хочу в булці",  callback_data="dish_bigjo_big_bulka")
...
async def r_medium(callback: types.CallbackQuery):
    builder = InlineKeyboardBuilder()
    builder.button(text="🌯 Сендвіч-Cheese-рол", callback_data="dish_cheese_roll_mid_lavash")
    builder.button(text="🍞 Тост з куркою",       callback_data="dish_toast_chick_mid_bulka")
    builder.button(text="🌯 Загнаний Койот",      callback_data="dish_coyote_big_lavash")
    builder.button(text="🔙 Назад",               callback_data="recommend")
    builder.adjust(1)
    await callback.message.edit_text(
...
async def r_light(callback: types.CallbackQuery):
    builder = InlineKeyboardBuilder()
    builder.button(text="🌯 Сендвіч-рол з шинкою", callback_data="dish_ham_roll_mid_lavash")
    builder.button(text="🌯 Сендвіч-Cheese-рол",   callback_data="dish_cheese_roll_mid_lavash")
    builder.button(text="🍞 Тост з шинкою",         callback_data="dish_toast_ham_mid_bulka")
    builder.button(text="🔙 Назад",                 callback_data="recommend")
    builder.adjust(1)
    await callback.message.edit_text(
